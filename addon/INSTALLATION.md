...
3. Create `.env` file (copy from `env.example` in the repository's `addon/` directory)
4. Create `profiles.json` file (copy from `profiles.json.example` in the repository's `addon/` directory)
...
# The env.example file is in your repository at: addon/env.example
nano .env
# Paste and edit the content from env.example

# Create profiles.json (you'll need to copy content from repository)
# The profiles.json.example file is in your repository at: addon/profiles.json.example
nano profiles.json
